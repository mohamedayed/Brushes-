const String _imagesPath = "assets/images";
const String _iconsPath = "assets/icons";

class AppImages {
  static const String blank = "$_imagesPath/blank.png";
  static const String emptyBooking = "$_imagesPath/empty_booking.svg";
  static const String placeholder = "$_imagesPath/placeholder.png";
  static const String signInBlurryBg = "$_imagesPath/sign_in_blurry_bg.png";
  static const String signInRedBg = "$_imagesPath/sign_in_red_bg.png";
}

class AppIcons {
  static const String bagFilled = "$_iconsPath/bag-filled.svg";
  static const String bell = "$_iconsPath/bell.svg";
  static const String bellFilled = "$_iconsPath/bell-filled.svg";
  static const String call = "$_iconsPath/call.svg";
  static const String camera = "$_iconsPath/camera.svg";
  static const String eye = "$_iconsPath/eye.svg";
  static const String eyeSlash = "$_iconsPath/eye_slash.svg";
  static const String facebook = "$_iconsPath/facebook.svg";
  static const String google = "$_iconsPath/google.svg";
  static const String heart = "$_iconsPath/heart.svg";
  static const String heartFilled = "$_iconsPath/heart-filled.svg";
  static const String home = "$_iconsPath/home.svg";
  static const String homeActive = "$_iconsPath/home_active.svg";
  static const String image = "$_iconsPath/image.svg";
  static const String languageFilled = "$_iconsPath/language-filled.svg";
  static const String location = "$_iconsPath/location.svg";
  static const String lock = "$_iconsPath/lock.svg";
  static const String logout = "$_iconsPath/logout.svg";
  static const String message = "$_iconsPath/message.svg";
  static const String messageActive = "$_iconsPath/message_active.svg";
  static const String messages = "$_iconsPath/messages.svg";
  static const String messagesFilled = "$_iconsPath/messages-filled.svg";
  static const String mobile = "$_iconsPath/mobile.svg";
  static const String ppFilled = "$_iconsPath/pp-filled.svg";
  static const String profile = "$_iconsPath/profile.svg";
  static const String profileActive = "$_iconsPath/profile_active.svg";
  static const String search = "$_iconsPath/search.svg";
  static const String send = "$_iconsPath/send.svg";
  static const String shop = "$_iconsPath/shop.svg";
  static const String shopActive = "$_iconsPath/shop_active.svg";
  static const String sms = "$_iconsPath/sms.svg";
  static const String star = "$_iconsPath/star.svg";
  static const String success = "$_iconsPath/success.svg";
  static const String themeFilled = "$_iconsPath/theme-filled.svg";
  static const String ticket = "$_iconsPath/ticket.svg";
  static const String todo = "$_iconsPath/todo.svg";
  static const String todoFilled = "$_iconsPath/todo-filled.svg";
  static const String todoActive = "$_iconsPath/todo_active.svg";
  static const String user = "$_iconsPath/user.svg";
  static const String walletFilled = "$_iconsPath/wallet-filled.svg";
}

class AppLotties {}
